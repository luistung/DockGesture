...
最终应用位于 `outputs/DockGesture.app`，便于传递的压缩包位于 `outputs/DockGesture.zip`。建议先把应用移动到“应用程序”文件夹，再首次打开；授予权限后不要再移动应用，否则 macOS 可能要求重新授权。

1. 打开 DockGesture，菜单栏会出现手指图标或警告图标。
2. 按菜单提示授予“辅助功能”和“输入监控”权限。
3. 如果授权后状态未自动变为“运行中”，退出并重新打开一次。
4. 如需开机自动运行，在菜单中勾选“登录时启动”。首次启用可能还需在“系统设置 → 通用 → 登录项”中批准。
